...
if ($buildApk) {
    ...
    # Export APK to dedicated APK folder
    $apkFolder = Join-Path $outputFolder "APK"
...
if ($buildAab) {
    ...
    # Export AAB to dedicated aab folder
    $aabFolder = Join-Path $outputFolder "AAB"
    if (!(Test-Path $aabFolder)) {
        New-Item -ItemType Directory -Path $aabFolder | Out-Null
    }
