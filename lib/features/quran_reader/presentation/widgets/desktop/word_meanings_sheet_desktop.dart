...
class WordMeaningItemDesktop {
  final String word;
  final String meaning;

  const WordMeaningItemDesktop({
    required this.word,
    required this.meaning,
  });
}

List<WordMeaningItemDesktop> _parseGhareeb(String raw) {
  final lines = raw.split('\n');
  final items = <WordMeaningItemDesktop>[];
  for (final line in lines) {
    ...
    if (colonIdx != -1) {
      ...
      items.add(WordMeaningItemDesktop(word: word, meaning: meaning));
    } else {
      items.add(WordMeaningItemDesktop(word: '', meaning: trimmed));
    }
  }
  return items;
}

class WordMeaningsSheetDesktop extends StatefulWidget {
  final VerseModel verse;

  const WordMeaningsSheetDesktop({
    super.key,
    required this.verse,
  });
  ...
  static Future<void> show(
      ...
      return showDialog(
        ...
        builder: (ctx) => Dialog(
          ...
          child: ConstrainedBox(
            ...
            child: WordMeaningsSheetDesktop(verse: verse),
          ),
        ),
      );
    }
    ...
    return showModalBottomSheet(
      ...
      builder: (ctx) => WordMeaningsSheetDesktop(verse: verse),
    );
  }

  @override
  State<WordMeaningsSheetDesktop> createState() =>
      _WordMeaningsSheetDesktopState();
}

class _WordMeaningsSheetDesktopState extends State<WordMeaningsSheetDesktop> {
  List<WordMeaningItemDesktop>? _meanings;
        ...
        child: Column(
          ...
          children: [
            ...
            if (_isLoading)
              ...
              _WordMeaningsEmptyStateDesktop(
                isArabic: isArabic,
                l10n: l10n,
              )
            else
              Flexible(
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    return _WordMeaningCardDesktop(
...
class _WordMeaningsHeaderDesktop extends StatelessWidget {
  final String surahName;
  final int ayahNumber;
  final bool isArabic;
  final AppLocalizations l10n;

  const _WordMeaningsHeaderDesktop({
...
class _WordMeaningsEmptyStateDesktop extends StatelessWidget {
  final bool isArabic;
  final AppLocalizations l10n;

  const _WordMeaningsEmptyStateDesktop({
    required this.isArabic,
    required this.l10n,
  });
...
class _WordMeaningCardDesktop extends StatelessWidget {
  final WordMeaningItemDesktop item;
  final int index;

  const _WordMeaningCardDesktop({
    required this.item,
    required this.index,
  });
