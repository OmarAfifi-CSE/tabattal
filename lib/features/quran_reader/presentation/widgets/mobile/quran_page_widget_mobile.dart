...
class _QuranPageWidgetMobileState extends State<QuranPageWidgetMobile> with SingleTickerProviderStateMixin {
  int? _activeVerseId;
  ...
  Rect _calculateVerseScreenRect(String verseKey, List<LineData> lines, Offset fallbackPosition) {
    ...
    if (minLine > 15) return Rect.fromCenter(center: fallbackPosition, width: 0.w, height: 0.h);

    final renderBox = _pageColumnKey.currentContext?.findRenderObject() as RenderBox?;
    if (renderBox == null) return Rect.fromCenter(center: fallbackPosition, width: 0.w, height: 0.h);
