...
abstract class QuranRemoteDataSource {
  Future<Map<String, dynamic>> getTafsirByVerse(
    int resourceId,
    String verseKey,
  );
  Future<TranslationModel> getTranslationByVerse(
    String verseKey, {
    int translationId = 20,
  });
  Future<List<TafsirModel>> getTafsirsByChapter(
    int chapterId, {
    int tafsirId = 16,
    int page = 1,
  });
  Future<List<TranslationModel>> getTranslationsByChapter(
    int chapterId, {
    int translationId = 20,
  });
  Future<Map<String, dynamic>> getTafsirByChapter(
    int resourceId,
    int chapterId, {
    int page = 1,
    int perPage = 300,
...
class QuranRemoteDataSourceImpl implements QuranRemoteDataSource {
  ...
  Future<List<TafsirModel>> getTafsirsByChapter(
    ...
    return apiClient.getJson(
      ...
      parse: (json) => (json['tafsirs'] as List? ?? [])
          .map((t) => TafsirModel.fromJson(t))
          .toList(),
    );
  }
  ...
  Future<List<TranslationModel>> getTranslationsByChapter(
    ...
    return apiClient.getJson(
      ...
      parse: (json) => (json['translations'] as List? ?? [])
