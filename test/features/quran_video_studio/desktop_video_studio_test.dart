...
void main() {
  ...
  testWidgets('Selecting custom video and then changing other settings does not throw or break', (tester) async {
    ...
    innerContext.read<VideoStudioBloc>().add(const VideoStudioCustomVideoSelected('test_video.mp4'));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 200));

    // Verify custom video is now in state
    expect(innerContext.read<VideoStudioBloc>().state.config.hasCustomVideo, isTrue);
    ...
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 200));
    expect(innerContext.read<VideoStudioBloc>().state.config.aspectRatio, VideoAspectRatio.landscape16x9);
    ...
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 200));
    expect(innerContext.read<VideoStudioBloc>().state.config.aspectRatio, VideoAspectRatio.square1x1);

    // 4. Try to toggle display mode
    innerContext.read<VideoStudioBloc>().add(const VideoStudioTextDisplayModeChanged(VideoTextDisplayMode.staticFull));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 200));
    expect(innerContext.read<VideoStudioBloc>().state.config.textDisplayMode, VideoTextDisplayMode.staticFull);

    // 5. Try to toggle options (card frame, tafsir, translation)
    innerContext.read<VideoStudioBloc>().add(const VideoStudioOptionToggled(showCardFrame: false));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 200));
    expect(innerContext.read<VideoStudioBloc>().state.config.showCardFrame, isFalse);

    // 6. Try to switch background back to gradient or photo
    innerContext.read<VideoStudioBloc>().add(const VideoStudioCustomVideoSelected(null));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 200));
    expect(innerContext.read<VideoStudioBloc>().state.config.hasCustomVideo, isFalse);
  });
